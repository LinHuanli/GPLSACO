#!/usr/bin/env bash
# 成本诊断、计时模板与生产GPU入口回归；目标卡先核查实时占用。
set -euo pipefail
project_root="$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")/.." && pwd)"
cd "$project_root"
gpu_uuid="${1:?Usage: bash scripts/run_profiling_checks.sh GPU-uuid}"
mkdir -p artifacts/gpu/profiling .tmp .cache/cuda
...
Path('artifacts/gpu/profiling/device.csv').write_text(gpu)
print(gpu.strip(), flush=True)
PY
export CUDA_VISIBLE_DEVICES="$gpu_uuid"
ctest --test-dir build/cuda --output-on-failure > artifacts/gpu/profiling/ctest.log 2>&1
GP_FACO_REQUIRE_NATIVE=1 GP_FACO_REQUIRE_CUDA=1 .venv/bin/python -m pytest -q \
    --basetemp="$project_root/.tmp/pytest-profiling" \
    > artifacts/gpu/profiling/pytest.log 2>&1
for sanitizer in memcheck synccheck racecheck; do
    /opt/cuda/bin/compute-sanitizer --tool "$sanitizer" --error-exitcode 42 \
        build/cuda/profiling_semantics "artifacts/gpu/profiling/$sanitizer.json" --small \
        > "artifacts/gpu/profiling/$sanitizer.log" 2>&1
done
cat artifacts/gpu/profiling/ctest.log artifacts/gpu/profiling/pytest.log
tail -n 1 artifacts/gpu/profiling/memcheck.log
tail -n 1 artifacts/gpu/profiling/synccheck.log
tail -n 1 artifacts/gpu/profiling/racecheck.log
