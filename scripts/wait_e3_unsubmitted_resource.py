#!/usr/bin/env python3
"""原UUID空闲后接续E3完整训练；只处理明确的未提交资源暂停，不重算实际返回。"""
...
ADMISSION_ERROR = "目标GPU占用未知或存在外来进程，尚未提交当前任务"
...
def main():
    ...
    with (PROJECT / ".tmp" / f"e3-unsubmitted-waiter-{request['gpu_uuid']}.lock").open(
        "a"
    ) as lease:
