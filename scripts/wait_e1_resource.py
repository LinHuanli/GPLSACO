#!/usr/bin/env python3
"""原UUID空闲后重启原E1完整CLI；只接续有明确证据的未提交待办，不重算实际返回。"""
...
ADMISSION_ERROR = "目标GPU出现外来进程；当前任务未提交，保留待办"
...
def run(request_path):
    ...
    command = [
        sys.executable,
        str(PROJECT / "scripts/research_e1.py"),
        "train",
        "--freeze",
        str(PROJECT / request["freeze"]),
        "--condition",
        request["condition"],
        "--evolution-seed",
        str(request["evolution_seed"]),
        "--gpu-uuid",
        request["gpu_uuid"],
        "--output",
        str(directory),
...
def main():
    ...
    with (PROJECT / ".tmp" / f"e1-waiter-{request['gpu_uuid']}.lock").open("a") as lease:
