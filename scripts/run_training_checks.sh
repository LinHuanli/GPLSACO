#!/usr/bin/env bash
# 训练任务、固定费用与GPU入口回归；目标卡先核查实时占用。
set -euo pipefail
project_root="$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")/.." && pwd)"
cd "$project_root"
gpu_uuid="${1:?Usage: bash scripts/run_training_checks.sh GPU-uuid}"
mkdir -p artifacts/gpu/gp-training .tmp .cache/cuda
exec 9>.tmp/gp-training-checks.lock
flock -n 9 || { echo '此项目已有训练接口检查在运行'; exit 2; }
...
Path('artifacts/gpu/gp-training/device.csv').write_text(gpu)
print(gpu.strip(), flush=True)
PY
export CUDA_VISIBLE_DEVICES="$gpu_uuid"
ctest --test-dir build/cuda --output-on-failure > artifacts/gpu/gp-training/ctest.log 2>&1
GP_FACO_REQUIRE_NATIVE=1 GP_FACO_REQUIRE_CUDA=1 .venv/bin/python -m pytest -q \
    --basetemp="$project_root/.tmp/pytest-gp-training" \
    > artifacts/gpu/gp-training/pytest.log 2>&1
cat artifacts/gpu/gp-training/pytest.log
