"""从已验证E1审计提取的流式面板/收据/DEAP重放；研究范围由独立入口核对。"""
...
from gp_faco.fitness import aggregate_panels, score_panel
from gp_faco.program_ir import Program, export_tree
from gp_faco.training import finite_or_none, json_value
from gp_faco.worker import SolveTask, content_hash, coordinate_hash
...
def audit_receipts(directory, state, protocol, settings, source, allowed_ids, complete):
    ...
    raw_paths = {p.stem: p for p in (directory / "raw_returns").glob("*.json")}
    actual_raw_returns = set()
    admissions = Counter()
    ...
    for key, digest in sorted(completed.items()):
        ...
        if returned:
            if protocol.graph_catalog is not None:
                require(key in raw_paths, "图任务实际返回缺少独立原始归档")
                raw = json.loads(raw_paths[key].read_text())
                require(
                    raw == {k: v for k, v in outcome.items() if k != "gpu_boundary_after"},
                    "GPU边界观察改变了真实返回，或归档与任务不一致",
                )
                actual_raw_returns.add(key)
        ...
        if protocol.graph_catalog is not None:
            require(
                d.get("graph_inputs")
                == protocol.graph_catalog.describe(
                    tuple(problems[name] for name, _ in d["problems"]), protocol.graph_prior_kind
                ),
                "实际准备/求解图与冻结目录不符",
            )
        for field, origin in (
    ...
    if complete and protocol.graph_catalog is not None:
        require(set(raw_paths) == actual_raw_returns, "独立原始返回含额外或未归集任务")
...
def verify_selection(directory, state, shortlist):
    ...
    require(
        ...
        and exported["scope"] == state["manifest"]["settings"]["scope"]
