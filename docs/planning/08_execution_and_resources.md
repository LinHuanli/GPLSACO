...
128×50=6,400个个体评价/演化重复，每个含500和1K两个规模工作波。如果每个规模的32个colony一次放下，有效预算总额度为 `6400*(B500+B1000)`；五重复乘5，另计实际初始化/调度、验证shortlist、失败重跑、Static/Rule调参和消融。缓存路径的冻结扣费并非每次实际消耗，末批超限又会增加实际运行时间，因此这个额度不是实际GPU墙钟下界。若每波容不下，按固定分波数再乘，不能把吞吐量当独占时延。
