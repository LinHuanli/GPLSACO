...
固定并发Engine先为整个面板建立节点ID顺序的廉价闭环，再准备候选与节点0最近邻＋checklist 2-opt初始解。初始改进只有成本严格更好才替换廉价解，等成本保留较早结果。`PreparedProblem`由固定迭代入口与批量Engine共用，因此此后固定迭代诊断也采用该规则；前序提交的历史pilot报告保留原身份，不混合汇总。

这是保留截止前incumbent所需的共同初始化规则，不能视为原生已有行为。缓存模式按廉价/昂贵阶段分别收费，昂贵阶段超时不能提交其解；全量准备的CPU路径仍有明确评价上限。见 [多实例与截止报告](../reports/2026-09-08_batch_engine.md)。正式共享变体的初始化必须一致，G4仍需冻结参数与硬件费用。
