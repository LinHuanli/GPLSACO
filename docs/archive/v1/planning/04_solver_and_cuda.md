...
显式spawn worker和外部fitness已实现并实测，见 [worker报告](../reports/2026-09-08_worker.md)。子进程启动时建立每规模固定形状Engine，任务输入冻结为无标签坐标、Program与replica列表。协调进程既不初始化CUDA，也不参与在线批次控制。静态缓存采用公开容量上限和任务边界重建；动态状态仍由每次原生评价重置。

任务身份含评价位置、程序/坐标/硬件协议hash；真实待完成Future是运行状态依据，客户端等待超时不触发重启。当前输入错误保留失败，进程故障显式传播；正式训练的固定重试政策与checkpoint恢复尚未实现。缓存费用、主机任务总时间和原生预算分别保留，不能把首次登记耗时免费省略，也不能把worker总时间称为kernel时间。
