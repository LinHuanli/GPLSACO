...
\begin{abstract}
本材料补充主稿的参数依据、开发标定、六个冻结individual的完整树结构、真实输入输出、逐seed结果和计算成本，并整理此前完成的实验。正式比较使用两种表示各3个GP seed、128个individual、50代训练及冻结测试。实验沿革单独记录旧版32蚂蚁实验、暂停轮次和工程加速；各阶段的设置与结果分别呈现。实验结果截至2026年9月10日。
\end{abstract}
...
主稿用短符号展示tree，本文用相同符号绘制所有冻结individual。表\ref{tab:symbols}给出与保存记录的对应，方便将数学表达式、真实输入和程序连接起来。图中内部节点为function，浅色叶子为terminal；ERC用$c_i$标记，并在树下列出完整FP32数值。
...
Monitoring与quick validation的ACO seed为17，full validation为17、29、41，两个规模的测试均使用17、29、41、53、67、79、97、109、127、149。全部六次训练共使用78\,643\,200\,000 FE，监控、选模和测试单独计数。训练面板每代变化，完整面板归属由来源索引中的配置与清单确定。

\subsection{构造、参考路径与繁殖的具体处理}
GPU构造使用权重$\tau_{ij}d_{ij}^{-\beta}$在主要候选中选择节点；主要候选不可用时，使用随后64个近邻构成的备用表及后备选择。候选表规定优先搜索范围。初始化从节点0出发生成最近邻路径，再执行FIFO检查表驱动的2-opt；候选邻域和移动评价上限使其终态不必是完整2-opt邻域的局部最优。

\FACO{}在合成数据上使用连续FP64欧氏距离，将依赖距离取整的KD-tree邻域实现替换为近邻列表；恒等2-opt移动赋零增益，边集合不变的3-opt移动被排除，以避免浮点误差触发虚假改善循环。该适配保留原生24条初始化路径及2-opt/3-opt初始化。GPU底座则采用2024后续实现的节点重定位构造及上述单路径初始化。原始整数距离的TSPLIB复现单独报告，三者的路径质量与耗时均按各自设置解释。

备选参考路径的边差异在64个节点上采样，优先选差异最大的合格archive路径；随后以路径质量和固定次序处理并列。区域0和1各自随机选择起点，再沿参考路径取连续片段；区域2从随机节点沿主要近邻图扩展。所有区域在动作执行前生成。动作切换参考路径后才重置平滑反馈，conditional representation的后续选择仍使用原执行前状态。

Mutation生成高度0--2的full子树；当被变异的树只有一个叶子时，使用高度1--3的生长变异。无法找到行为不同的第二父本时回退为mutation。每个新individual最多尝试25次variation，再尝试至多128次随机移民；仍无法满足约束则停止任务。Tournament的fitness并列随机处理，elites及报告排序则在fitness并列后按节点数和程序表示排序。实现使用DEAP的GP表示与相关工具\cite{fortin2012}。Tree-swapping相关工作见Zhang等\cite{zhang2018multitree}，本文的具体算子以本节和主稿描述为准。
...
\caption{固定开发面板上的在线monitoring。每5代评价一次冠军，16实例、ACO seed 17、H1000；细线为三个GP seed，粗线为均值。它与训练后执行的quick validation具有不同的面板和评价频率。}\end{figure}
