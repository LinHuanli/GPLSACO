...
| 20 | [正式E3与全量图准备](planning/20_e3_formal_freeze_and_graph_preparation.md) | 四条件×五seed的128×50训练、完整Static搜索、2,296实例准备与预定统计；[完整准备验收](reports/2026-09-09_e3_formal_preparation.md) |
...
新增空闲设备后的[23分组正式启动与前缀审计](reports/2026-09-09_e3_formal_launch.md)已完成；19个GP和四个Static实际运行，另一个GP由固定队列接续。此前21设备分配已由v2替代。随后一个GP发生实际GPU争用，已通过[保留原返回的恢复与独立审计](reports/2026-09-09_e3_resource_recovery.md)并继续原完整训练；争用计时单独分类。
