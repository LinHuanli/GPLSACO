...
SPEC = importlib.util.spec_from_file_location(
    "baseline_resource_wait", ROOT / "scripts/wait_baseline_resource.py"
)
...
def test_wait_and_repeat_only_explicit_pre_submission_pause(tmp_path, monkeypatch, final_phase):
    ...
    save_checkpoint(
        ...
        {
            "worker_protocol": hardware,
            "data": {"identity": {"config_path": "configs/baseline_tuning_v1.json"}},
        },
    )
    ...
    request = {
        ...
        "config": "configs/baseline_tuning_v1.json",
        "sources": {
            "scripts/wait_baseline_resource.py": file_hash(
                ROOT / "scripts/wait_baseline_resource.py"
            )
        },
    }
    ...
    class Child:
        ...
        def __init__(self, command, *, cwd, stdout, stderr):
            ...
            assert str(ROOT / "scripts/search_baselines.py") in command
            assert command[command.index("--config") + 1] == str(
                ROOT / "configs/baseline_tuning_v1.json"
            )
            self.stdout = stdout

        def wait(self):
            if len(calls) == 1:
                self.stdout.write("ValueError: " + MODULE.ADMISSION_ERROR + "\n")
                self.stdout.flush()
                return 1
